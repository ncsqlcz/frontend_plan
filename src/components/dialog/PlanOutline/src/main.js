...
const PlanOutline = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, PlanOutline.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
PlanOutline.setDefaults = defaults => {
  PlanOutline.defaults = defaults
}

// 显示弹框
PlanOutline.show = (title, options) => {
  ...
  return PlanOutline(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
