 ...
 * @Last Modified time: 2018-09-08 15:42:58
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="outsourcing_unit_number" label="客户编号" sortable></el-table-column>
      <el-table-column prop="outsourcing_unit_name" label="外协名称" sortable></el-table-column>
      <el-table-column prop="outsourcing_unit_site" label="外协地址" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.outsourcing_unit_id).forEach(id => {
          if (id === row.outsourcing_unit_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.outsourcing_unit_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.outsourcing_unit_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { outsourcing_unit_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await this.ajax('queryOutsourcingUnitMsgs', para)
        if (res.status === 0) {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.outsourcing_unit_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.outsourcing_unit_id === this.selectedIds[0]) {
              this.$nextTick(() => {
