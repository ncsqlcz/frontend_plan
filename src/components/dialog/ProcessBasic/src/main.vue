 ...
 * @Last Modified time: 2018-08-30 11:15:32
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="production_process_name" label="工序名称" sortable></el-table-column>
      <el-table-column prop="production_process_number" label="工序编号" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.plan_production_craft_path_id).forEach(id => {
          if (id === row.plan_production_craft_path_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.plan_production_craft_path_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.plan_production_craft_path_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { plan_production_craft_path_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await this.ajax('queryProcessMsgs', para)
        if (res.status === 0) {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.plan_production_craft_path_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.plan_production_craft_path_id === this.selectedIds[0]) {
              this.$nextTick(() => {
