 ...
 * @Last Modified time: 2018-09-27 11:36:48
 */
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.material_id).forEach(id => {
          if (id === row.material_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.material_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.material_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { material_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let para = Object.assign({'type':'all','templateType':'0'}, this.filters, this.params)
        let res = await this.ajax('queryMaterial', para)
        if (res.status === 0) {
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.material_id === this.selectedIds[0]) {
              this.$nextTick(() => {
