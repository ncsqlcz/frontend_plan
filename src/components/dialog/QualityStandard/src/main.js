...
const QualityStandard = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, QualityStandard.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
QualityStandard.setDefaults = defaults => {
  QualityStandard.defaults = defaults
}

// 显示弹框
QualityStandard.show = (title, options) => {
  ...
  return QualityStandard(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
QualityStandard.close = () => {
  this.action = ''
  this.visible = false
}
