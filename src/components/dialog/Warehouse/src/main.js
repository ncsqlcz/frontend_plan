...
const ProcessBasic = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, ProcessBasic.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
ProcessBasic.setDefaults = defaults => {
  ProcessBasic.defaults = defaults
}

// 显示弹框
ProcessBasic.show = (title, options) => {
  ...
  return ProcessBasic(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
