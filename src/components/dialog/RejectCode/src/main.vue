 ...
 * @Last Modified time: 2018-09-22 10:36:55
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="quality_unqualified_name" label="不良名称" sortable></el-table-column>
      <el-table-column prop="quality_unqualified_number" label="不良编号" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.quality_unqualified_id).forEach(id => {
          if (id === row.quality_unqualified_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.quality_unqualified_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.quality_unqualified_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { quality_unqualified_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await this.ajax('queryQualityUnqualified', para)
        console.log(para)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.list = res.map.qualityUnqualifiedDTOs
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.quality_unqualified_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.quality_unqualified_id === this.selectedIds[0]) {
              this.$nextTick(() => {
