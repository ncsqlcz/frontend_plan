...
const RejectCode = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, RejectCode.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
RejectCode.setDefaults = defaults => {
  RejectCode.defaults = defaults
}

// 显示弹框
RejectCode.show = (title, options) => {
  ...
  return RejectCode(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
