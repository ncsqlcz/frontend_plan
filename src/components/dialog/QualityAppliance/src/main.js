...
const QualityAppliance = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, QualityAppliance.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
QualityAppliance.setDefaults = defaults => {
  QualityAppliance.defaults = defaults
}

// 显示弹框
QualityAppliance.show = (title, options) => {
  ...
  return QualityAppliance(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
QualityAppliance.close = () => {
  this.action = ''
  this.visible = false
}

export default QualityAppliance
export { QualityAppliance }
