 ...
 * @Last Modified time: 2018-09-29 16:45:40
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="quality_appliance_number" label="检测工具编号" sortable></el-table-column>
      <el-table-column prop="quality_appliance_name" label="检测工具名称" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.quality_appliance_id).forEach(id => {
          if (id === row.quality_appliance_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.quality_appliance_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.quality_appliance_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { quality_appliance_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await this.ajax('queryQualityAppliance', para)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.list = res.map.qualityApplianceDTOs
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.quality_appliance_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.quality_appliance_id === this.selectedIds[0]) {
              this.$nextTick(() => {
