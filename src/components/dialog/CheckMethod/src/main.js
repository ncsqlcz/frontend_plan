...
const CheckMethod = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, CheckMethod.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
CheckMethod.setDefaults = defaults => {
  CheckMethod.defaults = defaults
}

// 显示弹框
CheckMethod.show = (title, options) => {
  ...
  return CheckMethod(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
