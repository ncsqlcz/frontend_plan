/*
 * @Author: liucz 
 * @Date: 2018-08-16 11:23:37 
 * @Last Modified by: liucz
 * @Last Modified time: 2018-09-26 11:43:19
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="quality_check_method_name" label="检测方式" sortable></el-table-column>
      <el-table-column prop="quality_check_method_description" label="检测方式说明" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.quality_check_method_id).forEach(id => {
          if (id === row.quality_check_method_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.quality_check_method_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.quality_check_method_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { quality_check_method_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let para = Object.assign({'type':'all'}, this.filters, this.params)
        let res = await this.ajax('queryQualityCheckMethod', para)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.list = res.map.qualityCheckMethodDTOs
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          this.selectedIds.forEach(id => {
            this.list.forEach((item, index) => {
              if (item.quality_check_method_id === id) {
                this.$nextTick(() => {
          ...
          this.list.forEach((item, index) => {
            if (item.quality_check_method_id === this.selectedIds[0]) {
              this.$nextTick(() => {
