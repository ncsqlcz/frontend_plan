 ...
 * @Last Modified time: 2018-09-08 11:42:39
 */
<template>
  <el-dialog :title="title || '选择工序' " :visible.sync="visible" class="__mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="customer_number" label="客户编号" sortable></el-table-column>
      <el-table-column prop="customer_name" label="客户名称" sortable></el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.customer_id).forEach(id => {
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.customer_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.customer_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { customer_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await this.ajax('queryCustomerMsgs', para)
        if (res.status === 0) {
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.customer_id === this.selectedIds[0]) {
              this.$nextTick(() => {
