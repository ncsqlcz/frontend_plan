...
const QualityProject = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, QualityProject.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
QualityProject.setDefaults = defaults => {
  QualityProject.defaults = defaults
}

// 显示弹框
QualityProject.show = (title, options) => {
  ...
  return QualityProject(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
QualityProject.close = () => {
  this.action = ''
  this.visible = false
}

export default QualityProject
export { QualityProject }
