 ...
 * @Last Modified time: 2018-09-30 17:35:25
 */

<template>
  <el-dialog :title="title || '选择' " :visible.sync="visible" class="__mes-dialog" width="50%">
    <el-col class="toolbar">
      <el-form :inline="true" :model="params" @submit.native.prevent class="text-right">
        <el-form-item>
          <el-select size="small" v-model="params.staffType" placeholder="选择方案" @change="getList">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="quality_template_number" label="方案编号" sortable></el-table-column>
      <el-table-column prop="quality_template_name" label="方案名称" sortable></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async getList () {
        ...
        let res = await this.ajax('queryQualityTemplate', para)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.list = res.map.templateList
